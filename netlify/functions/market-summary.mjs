...
const json = (body) => new Response(JSON.stringify(body),{status:200,headers:{'content-type':'application/json','cache-control':'no-store'}});
export default async () => {
  const now=new Date().toISOString(); const alphaKey=process.env.ALPHAVANTAGE_API_KEY;
  if(!alphaKey) return json({error:'ALPHAVANTAGE_API_KEY_NOT_SET',updatedAt:now,quotes:{},fundamentals:{},earnings:[],sources:[],dataQuality:{status:'NO_DATA'}});
  const symbols=['NVDA','MSFT','AAPL','AMZN','META','GOOGL'];
  const out={updatedAt:now,quotes:{},fundamentals:{},earnings:[],sources:['Alpha Vantage'],dataQuality:{status:'NO_DATA',quoteMode:'latest-trading-day-or-delayed; never labelled real-time',warnings:[]}};
  ...
  try{
    for(const s of symbols){
      ...
      if(price!==null) out.quotes[s]={price,change:toNum(x['09. change']),changePct:x['10. change percent']||null,asOf:x['07. latest trading day']||null,source:'Alpha Vantage GLOBAL_QUOTE',freshness:'daily_or_delayed'};
    ...
    out.earnings=parseCsv(text).slice(1,101).filter(r=>r.length>2).map(r=>({symbol:r[0]||null,date:r[2]||null,estimate:r[5]||null,source:'Alpha Vantage'})).filter(r=>r.symbol&&r.date);
    out.dataQuality.status=Object.keys(out.quotes).length===symbols.length?'PARTIAL_OR_DELAYED':'INCOMPLETE';
    if(Object.keys(out.quotes).length<symbols.length) out.dataQuality.warnings.push('QUOTE_DATA_INCOMPLETE');
    out.dataQuality.warnings.push('Real-time entitlement must be configured separately; current endpoint is not real-time.');
  }catch(err){out.error='UPSTREAM_DATA_ERROR';out.errorDetail=String(err?.message||err);out.dataQuality.status='UPSTREAM_ERROR';}
  return json(out);
};
